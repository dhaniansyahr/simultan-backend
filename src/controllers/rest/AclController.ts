...
export async function getMenuByAksesLevelId(c: Context): Promise<TypedResponse> {
        const aksesLevelId = c.req.param("aksesLevelId");

        const serviceResponse = await AclService.getMenuByAksesLevelId(Number(aksesLevelId));
