...
export async function validateAclDTO(c: Context, next: Next) {
        ...
        if (!data.acl || data.acl.length == 0) {
                ...
                for (const acl of data.acl) {
                        if (!acl.namaFitur)
                                invalidFields.push(
                                        generateErrorStructure(`acl[${index}].namaFitur`, "namaFitur cannot be empty")
                                );
                        if (!acl.actions || acl.actions.length == 0)
                                invalidFields.push(
                                        generateErrorStructure(`acl[${index}].actions`, "actions cannot be empty")
                                );
                        index++;
                }
        }

        if (!data.aksesLevelId)
                invalidFields.push(generateErrorStructure("aksesLevelId", "aksesLevelId cannot be empty"));
                        ...
                        if (!action) {
                                invalidFields.push(
                                        generateErrorStructure(
                                                "action",
                                                `Action ${actionName} not found for feature ${acl.namaFitur}`
                                        )
                                );
                        }
                }
